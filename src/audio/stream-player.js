...
(function (global) {
  ...
  function playEl(s) {
    const p = s.el.play();
    if (p && p.then) p.then(() => { s.unlocked = true; }, () => { /* 操作外なら次の操作で */ });
    else s.unlocked = true;
  }
  function unlockOutputSinks() {
    // 生きている全 sink の <audio> を、ユーザー操作のたびに確認する。
    //  ・まだ一度も鳴らせていない要素は、この操作で鳴らして「解錠」する(iOS/Android とも、一度ユーザー操作で
    //    play() した要素は以後 play()/pause() を操作無しで呼べる)
    //  ・解錠済みの要素は再生中(sinkActive)のときだけ鳴らし直す。止めているあいだ鳴らすと
    //    OS のロック画面/通知が「再生中」になってアプリの状態とずれる
    for (const s of outputSinkList) {
      if (s.unlocked && !sinkActive) continue;
      if (s.el.paused) playEl(s);
    }
  }
  ...
  function getOutputSink(audioCtx) {
    ...
    if (!s) {
      ...
      // 初回はたいてい再生ボタンのクリック中に来る(プレイヤー生成→getMasterGain)ので、その場で鳴らして解錠する
      s.unlocked = false;
      unlockOutputSinks();
    }
  ...
  // 再生中かどうかを出口へ知らせる。<audio> を再生中だけ鳴らし、止めているあいだは pause する
  // (OS のロック画面/通知の再生中/停止中をアプリと揃える。iOS は MediaSession.playbackState より
  //  <audio> 要素の状態を見る、と推定。2026-09-23 iPad で再生ボタンが逆に動く報告から)。
  // 解錠(最初のユーザー操作での play())が済んでいれば、操作外の play() も通る
  MML.Audio.setOutputSinkActive = (active) => {
    sinkActive = !!active;
    for (const s of outputSinkList) {
      if (active) { if (s.el.paused) playEl(s); }
  ...
  class MmlStreamPlayer {
    ...
    _createNode() {
      ...
      // (方針: MML作曲プレビューはNSF実ファイル再生と同じ2A03+拡張音源チップを
