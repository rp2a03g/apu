...
(function (global) {
  ...
  // 8ch使用時の波形領域(最悪ケース)。ppmck互換のバッファ番号(ppmckBufferNumbers)は
  ...
  // --- ppmck互換のバッファ番号(このツール自身は一切使わない) ---
  //
  // ppmck(mck/doc/mckc.txt の @N<num> の節)では、@N定義の先頭の数値は
  ...
  // 踏み合う組み合わせは、ppmckでは作曲者の責任で避ける必要があった。
  //
  // このツールは配置を自動で行う(allocate)ためバッファ番号を読み捨てるが、
  // *2mml変換の出力MMLをppmckへ持って行く人のために、書き出し時だけは
  // 「なるべく踏み合わない番号」を振っておく(準互換。時間軸を見ない静的な詰め込みなので
  // ppmckでの完全な正しさは保証しない)。
  N163Alloc.PPMCK_MAX_SAMPLE_LEN = 32; // ppmckが扱える最大波形長
  ...
  // 領域を空にして先頭から詰め直す(ppmckでも時分割で使い回すしかない状況なので、
  // 番号だけは有効な範囲に収める)。ppmckに無い33サンプル以上の波形は0を返す。
