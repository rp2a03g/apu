...
(function (global) {
  ...
  // ── 画面構成(2026-09-18 に再整理。方針「1画面に収める・グループを分かりやすく」) ──
  ...
  const CMD_CHIPS = () => [
    ...
    ['INST',  '@ OP MH N', T('音色/デューティ/VRC7音色/FDS変調/SUNSOFT 5Bノイズ周期')],
  ...
  // 見出し行のドラッグでダイアログを動かす(方針 2026-09-08)。動かした位置は次回も使う
  ...
  //   (方針: ボタンを押したら設定画面を出し、その中にコンバート開始ボタンを置く)
  function openModal(ctx) {
    ...
    // 「to MML」から開いたときは、見出し「変換設定」の右に「コンバート開始」を置く(方針 2026-09-09)。
    ...
    // ── 変換の進捗と結果ログ(方針 2026-09-09。位置はプリセット/テンポの直下 2026-09-18) ──
    ...
    // ── 3. 譜面の書き方(折りたたみ。既定は閉: 折りたたみ区画は全部閉じた状態が既定、方針 2026-09-18) ──
