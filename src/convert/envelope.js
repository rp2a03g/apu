...
(function (global) {
  ...
  //      mckBytecode が 'rest' ブレークポイントに分解する)、ppmckも q を音符+休符に分解する
  ...
  // 近似モード: 音符の間に残る休符/k トークン 1 個のコスト。読みやすさの目的(方針「音が繋がって
  ...
  //   ゲートの近似(2026-09-08、方針「音が繋がって演奏が読める方が大事」): 厳密一致だけだと
  ...
  MML.Convert.applyNoteEnd = function (scoreChannels, envReg, cmd, fpb, srcFps) {
    ...
    for (const ch of scoreChannels || []) {
      ...
      // @q<n>(ppmckの q8,-n)は独自拡張の @k<n> より優先(同じずれ 0 なら @k を 2 倍重くして同点を避ける)
