...
(function (global) {
  ...
  MML.NSF2MML.convert = function (writeLog, nsfBytes, header, songIndex, initRegs, initWrites, options) {
    ...
    // +高速アルペジオ→EN統合(2026-08-14)
    const evA = MML.Convert.mergeVibratoAndArpeggio(extractPulseEvents(timeline, 'p1', 1));
    const evB = MML.Convert.mergeVibratoAndArpeggio(extractPulseEvents(timeline, 'p2', 2));
    const evC = MML.Convert.mergeVibratoAndArpeggio(extractTriEvents(timeline));
