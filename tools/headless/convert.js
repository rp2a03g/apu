...
function parseCmdFlags(preset, cmdStr) {
  ...
  for (const kv of (cmdStr || '').split(',').filter(Boolean)) {
    ...
    const enumKey = ['DMC_RATE', 'PITCH_SA', 'RATE_MIX', 'DRUM_POLY', 'N163_WAVE', 'TUNING', 'TUNING_MIN', 'NOTE_END', 'GATE_TOL', 'LEN_SNAP', 'PART_ORDER', 'BARS_PER_LINE', 'CHANNEL_ORDER'].indexOf(k.trim()) >= 0;
