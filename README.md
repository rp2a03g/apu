...
- `src/mml/compiler.js` — `MML.Mml.compile(source)`
  ...
  - `SD<n>`/`SDOF`/`SDQR`（セルフディレイ=疑似エコー、@vr併用時のみ有効。リリースエンベロープが発動する区間のピッチを`<n>`個前のノートオンへ差し替える。SDQRはノートオン履歴のリセット。ppmck公式リファレンスの出力例と完全一致することを実測確認済み）
  - `SM`/`SMOF`（スムース、A/B/C対応。上位バイト=周期/周波数レジスタの位相リセットを伴う書き込みを「値が変化した時だけ」に切り替え、同オクターブ内のレガートでクリック音が出るのを防ぐだけの機能で、ピッチ自体はグライドしない。実機`CMD_SMOOTH`/`EFF2_SMOOTH_ENABLE`を実測確認）
  - `PS`（ポルタメント、A/B/C対応。`c PS g`のように次の音符そのものをグライド先として使う。実機`process_ps`/`pitchshift_setup`のb_div方式ステップ計算を移植した独自実装で、既存の`PT<target>,<duration>`（明示的な生レジスタオフセット指定）とは別物・共存する。★実機は最初の1サイクルだけ間隔にstepでなくdurationそのものを使うためグライドが次の音符へ食い込むことがある独特の挙動があるが、本実装は音符の時間内で目標へ収束する分かりやすい近似を採用している=ppmck公式リファレンスが明記する「PSコマンド後の音程は正確ではない」という注記の対象範囲を意図的に簡略化）
