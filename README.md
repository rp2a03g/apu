...
- `src/mml/compiler.js` — `MML.Mml.compile(source)`
  - 対応コマンド: `cdefgab`（音符, `+`/`#`/`-`で半音, 数値で音長, `.`で付点）, `r`（休符）, `n<num>[,<len>]`（直接音程指定,オクターブ2のCを0とした通し番号）, `o` `>` `<`（オクターブ）, `l`（デフォルト音長）, `v`（音量0-15）, `v+` `v-`（音量相対増減,省略時±1）, `q`（ゲートタイム0-8）, `@q<n>`（フレーム単位の早期ノートオフ）, `t`（テンポ、曲中の任意位置で変更可）, `K<n>`（移調・半音）, `@`（音色=デューティ比）, `&`（タイ）, `[ ]n`（繰り返し）, `[ ... | ... ]n`（最後の周回だけ`|`以降を演奏しない）, `{ ... }<len>`（連符・等分）, `EN<n>`/`ENOF`+`@EN<n>={...}`（ノートエンベロープ=高速アルペジオ、A/B/C/D対応、仕様通り厳密実装）, `EP<n>[,<delay>]`/`EPOF`+`@EP<n>={...}`（ピッチエンベロープ、A/B/C対応、実機`sound_pitch_enverope`と同一の生レジスタオフセット空間で厳密実装。`,<delay>`は**ppmck本家仕様には無いこのツール独自の拡張**＝発動を`<delay>`フレーム遅らせてから0番地からテーブルを辿る。省略時delay=0で従来通り）, `MP<n>`/`MPOF`+`@MP<n>={delay,speed,depth}`（ビブラート、A/B/C対応、実機`lfo_sub`/`warizan_start`を忠実移植した厳密実装。滑らかな三角波ではなく実機通りの階段状の変化になる）, `PT<target>,<duration>[,<delay>]`/`PTOF`（ポルタメント=単調な直線グライド、A/B/C対応。**ppmck本家ドキュメント（doc/mck.txt）には専用コマンドが無く「ピッチエンベロープ(EP)で代用してください」と明記されているため、これはppmck方言からの独自拡張**。targetはD<n>/EP<n>と同じ生レジスタオフセット空間、durationフレームで0からtargetへ到達し以降は最終値を保持。内部アルゴリズムはMPの`warizan_start`片道版(反転無し)と同一で6502ドライバ側もCEILDIVを共有）, `s<speed>,<depth>`（スイープ、A/B/C対応、近似実装）
  - 「近似実装」と明記したコマンド（`s`のみ）は、ppmck実機ドライバの該当ルーチン（speed→period変換式）まで追い切れておらず、方向・大まかな挙動は実測確認済みだが数値カーブの完全一致は未検証(詳細は`ROADMAP.md`フェーズ1.5参照)
