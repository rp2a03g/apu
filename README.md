...
original music and program. This repository contains no such files; use material you are
entitled to use.
...
- **PSF2（PlayStation 2）は読めない。** 対応しているのは PSF1（`.psf` / `.minipsf` / `.psflib`）まで
- **一部の機能はChromium系ブラウザでしか動かない**（ファイルピッカー・外部エディタ同期・ミニ操作窓）。
  Firefox / Safari でも本体は動くが、その機能だけが無効になる
- **PSFの音量とクリップを参照プレイヤーと突き合わせていない。** 他の形式はCD音源や
  VGMPlay の出力と照合して較正してあるが、PSFだけは手元に参照実装が無く未実施
...
計画していた機能はひととおり実装済み。今後の拡張候補は、NSF書き出しのループ最適化、
実機転送用のバイトコード最適化など。
