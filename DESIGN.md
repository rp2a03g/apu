...
- **出口を増やす**: エミュ再生 → NSF/SPC書き出し → 他ツール連携。聴かせる・見せる共有の主役は
  NSF(+ロール表示)。MMLソースは作者が出したいときだけ添えるもので、既定の配布物にはしない。
- **読む機能が独自性**: nsf2mml/spc2mml/kss2mml/gbs2mml/hes2mml による「既存曲→MML」の
  逆変換は他ツールにない資産。常に「作る」と「読む」の双方向を維持する。
  ただし他人の曲を裸にする機能なので、公開向けではなく**解析・学習・自分用アレンジの下地**と位置づける
  (ソースを公開したがる人は少ない、という前提。2026-09-16)。
...
- 将来サーバー機能を作る場合も、それは**上乗せのオプション**とし、
...
- `@DPCM` のサンプルも同じ: `.mml` に埋め込まず、ppmck と同じく **`.dmc` を `.mml` と同じフォルダに置く**。
  「開き直したら戻る」はブラウザ側の台帳(IndexedDB、`src/ui/dpcmStore.js`)で担い、ファイル形式には現れない。
  (2026-09-16 決定。Base64 埋め込み・URL参照・zip コンテナは、MMLソースを公開する需要が薄いこと
  ─ 聴かせる公開物は NSF ─ を踏まえて採らない)
...
- 作品の共有単位は **NSF/SPC等のバイナリが主**。.mmlファイルは作者が望むときだけ添える。
  URL共有をするなら「聴かせる/見せる(NSF+ロール)」の意味に限る。
...
| `src/share/` (新設) | URL共有(聴かせる/見せる用途) | コア |
